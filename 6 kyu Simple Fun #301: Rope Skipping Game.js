...
When he jumped 51 times                           --> 60 
*/
function tiaosheng(failedCount){
    var time;
    var jumps = 60;
    for (var i = 0; i < failedCount.length; i++) {
        var val = failedCount[i];
        time = val + 3 * i; //not including most recent rope fixing
        if (time < 57) {jumps = val - 3 * i; }
        else if (time >= 57 &&
          time <= 60) { return val;}
        else {return 60 - (3 * i); }
    }
    if (time < 57) {return 60 - 3 * (i);}
    return jumps;
}
