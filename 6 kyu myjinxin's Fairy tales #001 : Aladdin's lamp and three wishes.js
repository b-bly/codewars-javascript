/*
myjinxin's Fairy tales series #001:
Aladdin's lamp and three wishes
...
function threeWishes(parcel,wish1,wish2,wish3){
    ...
    for (var i = 1; i < arguments.length; i++) {
        num = arguments[i].match(/\d/g)[0];
        if (arguments[i].search(/I want \d+ wishes?/g) !== -1) {
            return [];
        }
        if (arguments[i].search(/s$/g) !== -1) {
            wish = arguments[i].replace(/I want \d+ (.+)s/g, replacer);
        } else {
        wish = arguments[i].replace(/I want \d+ (.+)s?/g, replacer);
        }
        index = parcel.indexOf(wish);
        
        if (index >= 0) {
            while (num > 0) {
                parcel.splice(index, 0, wish);
                num--;
            }
        } else { 
            while (num > 0) {
