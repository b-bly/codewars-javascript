...
var freewayGame = function(distKmToExit, mySpeedKph, otherCars) {
    var distAheadOfMe, passTime;
    var totalTime = distKmToExit / mySpeedKph;
    var score = 0;
    var car = otherCars;
    
    for (var i = 0; i < otherCars.length; i++) {
        distAheadOfMe = car[i][0]/60*car[i][1];
        passTime = distAheadOfMe/(car[i][1] - mySpeedKph);
        ...
        else if ((totalTime - passTime) > 0 && otherCars[i][0] > 0 &&
            passTime > 0) {
            score -= 1;
        }
    }
